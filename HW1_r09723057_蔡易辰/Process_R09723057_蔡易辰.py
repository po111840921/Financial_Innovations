...
class Process(object):
    ...
    def preprocessing(self):
        ...
        # 法一：使用datetime方式將str轉datetime
        # 法二：使用.to_datetime(self.data['date'], format="%d.%m.%Y %H:%M:%S.%f")的方式
        self.data['timestamp'] = self.data['date'].apply(lambda x: datetime.datetime.strptime(str(x), "%d.%m.%Y %H:%M:%S.%f"))
        #轉成index方便後續使用
